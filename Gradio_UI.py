...
class GradioUI:
    ...
    def interact_with_agent(self, prompt, messages):
        ...
        # Bypass code agent only for pure greetings
        chitchat_greetings = [
            r'^hi$', r'^hello$', r'^hey$', r'^how are you$', r'^good morning$', r'^good evening$', r'^good afternoon$',
            r'^(hi|hello|hey)[.! ]*$', r'^greetings[.! ]*$', r"^what's up[.! ]*$", r'^yo[.! ]*$'
        ]
        if any(re.match(pattern, prompt.strip(), re.IGNORECASE) for pattern in chitchat_greetings):
            messages.append(gr.ChatMessage(role="user", content=prompt))
            # Direct LLM call
            response = self.agent.model(prompt)
